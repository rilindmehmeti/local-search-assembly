...
# Sequential execution script for v5.cpp
# Runs 10 instances sequentially (one after another) as daemon
# Usage: ./scripts/run_sequential.sh [all v5.cpp arguments]
#        ./scripts/run_sequential.sh --daemon [all v5.cpp arguments]  # Run as daemon
...
echo -e "${GREEN}=== Sequential Execution Script ===" 
...
if [ "$DAEMON_MODE" == true ]; then
    DAEMON_LOG="${LOG_DIR}/sequential_daemon_$(date +%Y%m%d_%H%M%S).log"
    nohup "$0" "${ARGS[@]}" > "$DAEMON_LOG" 2>&1 &
    DAEMON_PID=$!
    echo -e "${GREEN}Sequential daemon started with PID: ${DAEMON_PID}${NC}"
...
echo -e "  Sequential runs: ${NUM_RUNS}"
echo -e "  Map: ${MAP_NAME}"
echo -e "  Log directory: ${LOG_DIR}/"
echo -e "  Arguments: ${ARGS[*]}"
echo -e "  Timeout: 1 week (604800 seconds)"
echo ""
...
run_instance() {
    ...
    local log_file="${LOG_DIR}/seq${run_num}_${MAP_NAME}_${timestamp}.log"
    local err_file="${LOG_DIR}/seq${run_num}_${MAP_NAME}_${timestamp}.err"
    ...
    echo "[Run ${run_num}/${NUM_RUNS}] Starting at $(date)" >> "$log_file"
    if [ "$has_seed" == true ]; then
        echo "  Using seed offset: +${run_num}" >> "$log_file"
    fi
    echo -e "${YELLOW}[Run ${run_num}/${NUM_RUNS}] Starting...${NC}"
    ...
    if [ -n "$timeout_cmd" ]; then
        if $timeout_cmd ./bin/v5 "${run_args[@]}" >> "$log_file" 2>> "$err_file"; then
            ...
            echo "[Run ${run_num}/${NUM_RUNS}] Completed successfully at $(date) (duration: ${hours}h ${minutes}m ${seconds}s)" >> "$log_file"
            echo -e "${GREEN}[Run ${run_num}/${NUM_RUNS}] Completed successfully (${hours}h ${minutes}m ${seconds}s)${NC}"
            echo -e "  Log: ${log_file}"
            return 0
        else
            local exit_code=$?
            ...
            local minutes=$(((duration % 3600) / 60))
            local seconds=$((duration % 60))
            echo "[Run ${run_num}/${NUM_RUNS}] Failed with exit code ${exit_code} at $(date) (duration: ${hours}h ${minutes}m ${seconds}s)" >> "$log_file"
            echo "Error details:" >> "$err_file"
            echo "Exit code: ${exit_code}" >> "$err_file"
            echo -e "${RED}[Run ${run_num}/${NUM_RUNS}] Failed with exit code ${exit_code} (${hours}h ${minutes}m ${seconds}s)${NC}"
            echo -e "  Log: ${log_file}"
...
echo -e "${BLUE}Starting ${NUM_RUNS} sequential runs...${NC}"
echo ""
...
# Run each instance one after another
for i in $(seq 1 $NUM_RUNS); do
    echo -e "${BLUE}=== Run ${i}/${NUM_RUNS} ===${NC}"
    
    if run_instance $i; then
        SUCCESS=$((SUCCESS + 1))
        echo -e "${GREEN}✓ Run ${i} finished successfully${NC}"
    else
        FAILED=$((FAILED + 1))
        echo -e "${RED}✗ Run ${i} failed${NC}"
    fi
    
    # Small delay between runs to avoid file system issues
    if [ $i -lt $NUM_RUNS ]; then
        echo -e "${YELLOW}Waiting 2 seconds before starting next run...${NC}"
        sleep 2
    fi
    echo ""
done
...
echo -e "Total duration: ${TOTAL_HOURS}h ${TOTAL_MINUTES}m ${TOTAL_SECONDS}s"
...
if [ $FAILED -gt 0 ]; then
    ...
    for err_file in "${LOG_DIR}"/seq*_${MAP_NAME}_*.err; do
